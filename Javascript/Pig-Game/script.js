...
//it is for fetching ids and works same as querySelector
//the only major difference is that getElementById is little bit faster than querySelector
...
const switchPlayer = function () {
  ...
  //toggle will either add the class(not) or remove(present) the class (if the cls is present or not)
...
btnHold.addEventListener('click', function () {
  ...
  if (playing) {
    ...
    if (scores[activePlayer] >= 100) {
      //if yes :Finish the game ---> resetting all the things
      playing = false;
      diceEl.classList.add('hidden');
      document
